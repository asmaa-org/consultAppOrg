...
class SenderServices {
  ...
  Future<ApiResponse> updateSender(
      Sender oldSender, String name, mobile, address, int categoryId) async {
    ApiResponse apiResponse = ApiResponse();
    try {
      final response =
          await http.put(Uri.parse("$senderApi${oldSender.id}"), headers: {
        'Accept': 'application/json',
        'Authorization': 'Bearer $token',
      }, body: {
        'name': name,
