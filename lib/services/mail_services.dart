...
class MailServices {
  ...
  Future<ApiResponse> updateMail(Mail mail, int statusId, String decision,
      finalDecision, List<int> tags, List<Map> activities) async {
    //activities map: [{"body":"any text","user_id" : 1}]
    ApiResponse apiResponse = ApiResponse();
    try {
      final response =
          await http.put(Uri.parse("$singleMailApi${mail.id}"), headers: {
        'Accept': 'application/json',
        'Authorization': 'Bearer $token',
      }, body: {
        'decision': decision,
  ...
  Future<ApiResponse> storeAttachment(int attachment, int mailId) async {
    ApiResponse apiResponse = ApiResponse();
    try {
      final response = await http.post(Uri.parse(attachmentApi), headers: {
        'Accept': 'application/json',
        'Authorization': 'Bearer $token',
      }, body: {
